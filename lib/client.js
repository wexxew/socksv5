...
function Client(options) {
  ...
  this._socksConnected  = false; // first connecting to socks server
  this._ready = false;           // then connecting to host
  ...
  if (options && options.proxyTimeout) {
    this._sock.setTimeout(options.proxyTimeout)
  }
...
Client.prototype._onConnect = function() {
  ...
  self._socksConnected = true;


  if (self._options && self._options.timeout) {
    self._timeoutToHost = setTimeout(function(){
      if ( self._ready ) return

      let error = new Error;
      error.code = 'ETIMEOUT'
      error.message = 'Connecting from socks to host timeout'
  ...
  parser.on('method', function(method) {
    ...
    if (self._timeoutToHost) clearTimeout(self._timeoutToHost)
...
Client.prototype._onTimeout = function() {
  ...
  if (self._socksConnected) return;

  let error = new Error;
  error.code = 'ESOCKSTIMEOUT'
